...
StatusCode GamPiPiEE::execute() {
	...
	else if(runNo<0&&mc_cor)
	{
		...
		GamPiPiEE::calibration(mdcTrk_ep,wep_zHel,1);//1:e
		GamPiPiEE::calibration(mdcTrk_em,wem_zHel,1);//1:e
	...
	else if(runNo<0&&mc_cor)
	{
		...
		GamPiPiEE::calibration(mdcTrk_pip,wpip_zHel,0);//0:pi
		GamPiPiEE::calibration(mdcTrk_pim,wpim_zHel,0);//0:pi
...
void GamPiPiEE::corset(HepSymMatrix &V, HepMatrix &C, int n)
{
	//cout<<"v="<<V<<endl;
	//cout<<"c="<<C<<endl;
	double sum;
...
void GamPiPiEE::corgen(HepMatrix &C, HepVector &x, int n)
{
	int i,j;
	int nmax = 100;
...
//**********************************************
void GamPiPiEE::calibration(RecMdcKalTrack *trk , HepVector &wtrk_zHel, int n )
{
	...
	if(trk->charge()>0 && n==0){
		...
		GamPiPiEE::corset(wpip_zcal,wpip_zerrc,3);
		HepVector wpip_zgen(3,0);
		GamPiPiEE::corgen(wpip_zerrc,wpip_zgen,3);
	...
	if(trk->charge()<0 && n==0)
	{
		...
		GamPiPiEE::corset(wpim_zcal,wpim_zerrc,3);
		HepVector wpim_zgen(3,0);
		GamPiPiEE::corgen(wpim_zerrc,wpim_zgen,3);
		wtrk_zHel[0] = trk->getZHelix()[0];
	...
	if(trk->charge()>0 && n==1)
	{
		...
		GamPiPiEE::corset(wep_zcal,wep_zerrc,3);
		HepVector wep_zgen(3,0);
		GamPiPiEE::corgen(wep_zerrc,wep_zgen,3);
	...
	if(trk->charge()<0 && n==1)
	{
		...
		GamPiPiEE::corset(wem_zcal,wem_zerrc,3);
		HepVector wem_zgen(3,0);
		GamPiPiEE::corgen(wem_zerrc,wem_zgen,3);
		wtrk_zHel[0] = trk->getZHelixE()[0];
