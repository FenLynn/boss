...
#include "GamPiPiKKAlg/GamPiPiKK.h"
...
int Ncut9 = 0;
/////////////////////////////////////////////////////////////////////////////
GamPiPiKK::GamPiPiKK(const std::string& name, ISvcLocator* pSvcLocator) :
	Algorithm(name, pSvcLocator) {
		//Declare the properties
		declareProperty("EnergySpread",  m_EnergySpread = 0.0008);
		declareProperty("OutputFileName",  m_OutputFileName = "GamPiPiKK_test.root");
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
StatusCode GamPiPiKK::initialize(){
...
StatusCode GamPiPiKK::execute() {
	...
	for(int i = 0; i < nGood; i++) {
		...
		if(_charge==1){
			if((prob[i][2]>prob[i][1])&&(prob[i][2]>prob[i][0])) {flag_ep=i;N_eppid++;}
			else flag_pip=i;
		}
		else if(_charge==-1){
			if((prob[i][2]>prob[i][1])&&(prob[i][2]>prob[i][0])) {flag_em=i;N_empid++;}
			else flag_pim=i;
		}
		else continue;
	}
	...
	HepLorentzVector ep_un = mdcTrk_ep->p4(mk);
	HepLorentzVector em_un = mdcTrk_em->p4(mk);
	...
	if(runNo>0 || (runNo<0&&(mc_cor==0))){
		wvepTrk = WTrackParameter(mk, mdcTrk_ep->getZHelixK(), mdcTrk_ep->getZErrorK());  
		wvemTrk = WTrackParameter(mk, mdcTrk_em->getZHelixK(), mdcTrk_em->getZErrorK());
	}	
	else if(runNo<0&&mc_cor)
	{
		HepVector wep_zHel(5,0);HepVector wem_zHel(5,0);
		GamPiPiKK::calibration(mdcTrk_ep,wep_zHel,1);//1:e
		GamPiPiKK::calibration(mdcTrk_em,wem_zHel,1);//1:e
	...
	else if(runNo<0&&mc_cor)
	{
		...
		GamPiPiKK::calibration(mdcTrk_pip,wpip_zHel,0);//0:pi
		GamPiPiKK::calibration(mdcTrk_pim,wpim_zHel,0);//0:pi
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
StatusCode GamPiPiKK::finalize() {
	NbInfo->Fill();
	saveFile->cd();
	TreeAna->Write();
	if(m_saveCutFlow == 1) NbInfo->Write();
	saveFile->Close();
	cout<<endl<<"Finalize jpsi-> eta' phi,phi->K+K-  ,eta'->gam pi+ pi-"<<endl;;
...
void GamPiPiKK::corset(HepSymMatrix &V, HepMatrix &C, int n)
{
	//cout<<"v="<<V<<endl;
	//cout<<"c="<<C<<endl;
	double sum;
...
void GamPiPiKK::corgen(HepMatrix &C, HepVector &x, int n)
{
	int i,j;
	int nmax = 100;
...
//**********************************************
void GamPiPiKK::calibration(RecMdcKalTrack *trk , HepVector &wtrk_zHel, int n )
{
	...
	if(trk->charge()>0 && n==0){
		...
		GamPiPiKK::corset(wpip_zcal,wpip_zerrc,3);
		HepVector wpip_zgen(3,0);
		GamPiPiKK::corgen(wpip_zerrc,wpip_zgen,3);
	...
	if(trk->charge()<0 && n==0)
	{
		...
		GamPiPiKK::corset(wpim_zcal,wpim_zerrc,3);
		HepVector wpim_zgen(3,0);
		GamPiPiKK::corgen(wpim_zerrc,wpim_zgen,3);
		wtrk_zHel[0] = trk->getZHelix()[0];
	...
	if(trk->charge()>0 && n==1)
	{
		...
		GamPiPiKK::corset(wep_zcal,wep_zerrc,3);
		HepVector wep_zgen(3,0);
		GamPiPiKK::corgen(wep_zerrc,wep_zgen,3);
	...
	if(trk->charge()<0 && n==1)
	{
		...
		GamPiPiKK::corset(wem_zcal,wem_zerrc,3);
		HepVector wem_zgen(3,0);
		GamPiPiKK::corgen(wem_zerrc,wem_zgen,3);
		wtrk_zHel[0] = trk->getZHelixE()[0];
